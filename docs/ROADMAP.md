...
| v1.0.0 | Production Network | Stable production, reputation system, audit, launch | 6 | ⬜ Planned |
...
| 5 | Update architecture.md | Doc | Add planned Layer 4 (Intelligence): Consensus Inference, Smart Router, ARIA-LM. Mark planned layers as "Planned". | `docs/architecture.md` |
| 6 | Start Whitepaper v2 (outline) | Doc | **NEW** — Structured outline of WP v2 covering: extended architecture, Consensus Inference, ARIA-LM, agentic app vision. Not full content — just the validated structure. | `docs/whitepaper-v2-outline.md` |
...
| 5 | Testnet tokens (mock) | Code | **NEW** — Test contribution tracking to validate reputation mechanics. Counters visible in dashboard. | `aria/node.py`, `aria/ledger.py`, `desktop/` |
...
| 5 | Airdrop snapshot | Infra | **NEW** — Snapshot of testnet contributions: uptime, inferences, energy. Base for early adopter recognition. | `aria/ledger.py`, `scripts/` |
...
| 3 | Write Whitepaper v2 | Doc | **NEW** — Complete WP v2: extended architecture (5 layers), Consensus Inference, ARIA-LM, reputation system, agentic app vision, competitor comparison. PDF + website. | `ARIA_Whitepaper_v2.pdf`, `docs/` |
| 4 | Website redesign v2 | Web | **NEW** — New landing page: "local Claude Desktop" pitch, ARIA-LM section, reputation system, visual roadmap, download page. Multi-page. | GitHub Pages (complete redesign) |
...
| 6 | Distillation API frontier → training | Code | **NEW** — Pipeline: prompt → frontier API (user key) → high-quality response → local gradient computation → network aggregation. Only the gradient leaves the node. Contribution score 15-20× base. | `aria/distillation.py` (new), `aria/api_bridge.py` |
...
## v1.0.0 — Production Network — Stable & Scalable ⬜

| # | Task | Type | Detail | Files |
|---|------|------|--------|-------|
| 1 | Distributed KV-Cache across P2P network | Code | Share KV-cache across nodes. Collaborative context extension. | `aria/kv_cache.py` (new), `aria/node.py` |
| 2 | Context Relay: map-reduce for long documents | Code | Split long documents across nodes. Map-reduce style processing. | `aria/context_relay.py` (new) |
| 3 | Smart Inference Routing | Code | Intelligent routing: local first, then P2P network, then cloud API fallback. | `aria/router.py`, `aria/node.py` |
| 4 | Universal API Bridge | Code | Drop-in replacement for OpenAI API. Works with any OpenAI-compatible application. | `aria/api_bridge.py`, `aria/api.py` |
| 5 | Production security hardening | Infra | P2P network audit. Bug bounty program. Third-party security review. | `docs/threat-model.md`, `docs/audit-report.md` (new) |
| 6 | Production deployment | Com | Stable production network for public use. Launch campaign: HN, Reddit, Twitter, Product Hunt. | `docs/PUBLICATION_POSTS.md` |
...
| 2 | Full Computer Use (Level 2+3) | Code | **MOVED** — Full local vision pipeline + frontier routing for complex tasks. Mouse/keyboard control via enigo (Rust). Improvement feedback loop. *Moved from v1.0.0 to keep production launch focused.* | `desktop/src-tauri/src/computer_use.rs` (new) |
